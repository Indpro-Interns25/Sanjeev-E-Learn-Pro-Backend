...
class LessonModel {
  ...
  static async update(id, { title, content, position }) {
    ...
    const { rows } = await pool.query(
      'UPDATE lessons SET title = COALESCE($1, title), content = COALESCE($2, content), order_index = COALESCE($3, order_index), updated_at = CURRENT_TIMESTAMP WHERE id = $4 RETURNING *',
